...
%% Note
%  Ï£? ?ä§?éô?ä∏?üº?ô∏?óê Î∂??àò?†Å?ù∏ ?ä§?éô?ä∏?üº?ù¥ ?ÉùÍ∏? Í≤ΩÏö∞ IQ ?ç∞?ù¥?Ñ∞ ?åå?òï?ùÑ ?Ç¥?é¥Î≥¥Í≥† 
%  ?åå?òï?ù¥ Í∏âÍ≤©?ïòÍ≤? Î≥??ïò?äî Î∂?Î∂ÑÏóê?Ñú ?åå?òï?ùò ?úÑ ?ïÑ?ûòÍ∞? Î∞îÎ?åÏóà?äîÏß? ?Ç¥?é¥Î≥? Í≤?.  
%  ?õê?ù∏ : overflow
%  ?ï¥Í≤? : ?ã†?ò∏ ?†Ñ?†•?ùÑ ?ÇÆÏ∂òÎã§. 
...
%
% Ï£ºÏùò : ?ûÑÍ≥ÑÏπòÎ≥¥Îã§ ?Å∞ ?ä§?éô?ä∏?üº?óê?Ñú ?ã†?ò∏?Ñ±Î∂ÑÏô∏?ùò Í≤ÉÏù¥ ?è¨?ï®?ê†?ïå ?ò§Î•òÍ? ?Çú?ã§.
%
...
%     if noiseFloor > th1 % ?ã†?ò∏ ???ó≠?è≠ similar to fs/2
...
%         end
    ...
    % ???ó≠?è≠?Ç¥ Ï£ºÌåå?àò Îπ? ÏµúÎ? ?Ç¨?ù¥ Í∞ÑÍ≤© 500Hz/33Hz  = 151      
    % ?õêÏπòÏïä??  ?ä§?éô?ä∏?üº ?Ñ±Î∂ÑÏù¥ ?ã§?àò?ùºÎ©? Ï§ëÍ∞ÑÍ∞íÎ??Ñ∞ Î∞ñÏúºÎ°? ?ÇòÍ∞?Î©¥ÏÑú Î™®Îëê ?†úÍ±∞Ìïú?ã§.
...
try   
    % Î∞òÎìú?ãú ?ã†?ò∏?ùò Ï£ºÌåå?àò ???ó≠Îß? Ï∂îÍ? ?êò?ñ¥?ïº ?ïú?ã§.
    sam_freq                = handles.sam_freq;
    BWSampleIdx             = handles.BWSampleIdx;
    freqAxis                = handles.freqAxis;     
    ...
    switch adFlag
        ...
        case 'Digital'
            ...
            elseif coarseSamplesPerSymbol > spsUpperBound  % what is the proper decimation rate                
                ...
                % ?òÑ?û¨?ùò ?ç∞?ãúÎ©îÏù¥?Öò?ú®?ùÑ constantÎ≥¥Îã§ ?Å∞ 2?ùò Î∞∞ÏàòÎßåÌÅº ?äò?†§?ïº ?ïú?ã§.
...
%     isFSK             = handles.isFSK;
    ...
    if strcmp(linearFlag,'nonLinear')   
        ...
        % ?òµ?Öã Ï∂îÏ†ï?ïòÍ∏? ?†Ñ?óê Pxx_dB?ùº?Ñú ?†ú??Î°? ?ïà?êú?ã§.
        % ?ÉàÎ°úÏö¥ ?ä§?éô?ä∏?üº Í≥ÑÏÇ∞?ù¥ ?ïÑ?öî       
        power = 1;
        numTones = 0;
...
%     		// General information
% 			// 1) ?Ñ†?òïÎ≥?Ï°? ?ã†?ò∏?ùò ???ó≠?è≠?? ?ã¨Î≥ºÏú®?ùò 2.5Î∞∞Î?? ?ÑòÏß? ?ïä?äî?ã§.
% 			// 2) ÎπÑÏÑ†?òïÎ≥?Ï°? ?ã†?ò∏?ù∏ FSK?ùò ???ó≠?è≠?? ?ã¨Î≥ºÏú®?ùò 8.5Î∞∞Î?? ?ÑòÏß? ?ïä?äî?ã§.
% 			//    ex) ?ã¨Î≥ºÏú®Í≥? ???ó≠?è≠?ùò ÎπÑÍ? Í∞??û• ?Å∞ ?îÑÎ°úÌÜ†ÏΩúÏ? POCSAG?úºÎ°? 6.8Î∞? ?ù¥?ã§.
% 			// ?î∞?ùº?Ñú ?ã¨Î≥ºÏú®Í≥? ???ó≠?è≠?ùò ÎπÑÎ?? Í≥ÑÏÇ∞?ïòÎ©? FSK?? FM?ùÑ Íµ¨Î∂Ñ?ï† ?àò ?ûà?ã§.     
...
%         if isFSK>0 % ?ã¨Î≥ºÏú® ?Ñ† ?ä§?éô?ä∏?üº?ùò ?Å¨Í∏∞Ïóê ?î∞?ùº FSK Î∂ÑÎ•ò ¬âÔ¶??ó§
                 % ?Ü§ ?ä§?éô?ä∏?üº Í≤?Ï∂úÏóê ?ã§?å®?ïú Í≤ΩÏö∞
...
% %                   load refData_FromGenerator_150203.mat % 2ASK ?†úÍ±?
%           load refData_FromGenerator_150407.mat % cc20 Ï∂îÍ?, gammax_max, circular statistics ?†úÍ±?
%     otherwise
% end
...
function [NumPeaks, sortedLocs, threshold]=...
    peakDetection(inputSig,MINPEAKDIST,threshold)
%%[PEAKVALUE, NUMERICPEAKLOCATIONS]=MYCFAR_V3(CC,obj)
%
% Detect peaks in data based on CA-CFAR algorithm and locals maximum.
%
% See also, phased.CFARDetector, myGet1DTrainingIdx, findpeaks
%
% Revision History
% Date              Description
% 2013.12.09        myGet1DTrainingIdx ºˆ¡§ Date  
% 2014.04.23        entire spectrum
% 2014.04.24        threshold is set based on histogram
% 2014.05.15        limit the windowsize 
% 2014.05.24        Modify threshold in histogram
%                   Add new rule
% 2014.05.26        Add Param.MinimumHeight 
% 2014.08.11        Edit local maximum rule
% 2015.01.03        
% 2015.09.04        Edit Local max. rule

% Tongbang
% http://tongbang.cbnu.ac.kr/index.html

MINPEAKDIST = floor(MINPEAKDIST);
inputSig = inputSig(:);

[nSamp, pNumChannels] =size(inputSig);
sigIdx= 1 : nSamp;
% figure; plot(inputSig)

%% Threshold rule
% Find samples greater than threshold
binaryPeakIndex = inputSig >= threshold;
THRpeakValue    = inputSig(binaryPeakIndex);
THRpeakLocation = sigIdx(binaryPeakIndex);
...
[pks, idx] = sort(THRpeakValue, 'descend');
% sorted locations     
locs = THRpeakLocation(idx); 
idelete = false(1, length(locs)); 
for i=1:length(locs),
    if ~idelete(i),        
        % If the peak is not in the neighborhood of a larger peak, find
        % secondary peaks to eliminate.
        lowerLcos = locs(i) - floor(MINPEAKDIST); 
        if lowerLcos < 1,     lowerLcos = 1; end
        
        UpperLocs = locs(i) + floor(MINPEAKDIST); 
        if UpperLocs > nSamp, UpperLocs = nSamp; end

        % Check whether current peak is local maximum or not               
        window = [lowerLcos : locs(i)-1, locs(i)+1 : UpperLocs].';
        
        isNotLocalMax = sum(inputSig(window) > inputSig(locs(i)));
        
        % Eleminate samples within window
        condition = (locs >= lowerLcos) & (locs <= UpperLocs);
        idelete = idelete | condition ;       
        
        if isNotLocalMax>0
            % Delete current sample
        else
%             minPeakHeight   = 1.5;
%             if  inputSig(locs(i)) < minPeakHeight 
                %idelete(i) = 1;      % Delete current sample 
%             else
                idelete(i) = 0; % Keep current sample
%             end
        end
        
    end
end
pks(idelete) = [];          % peaks values
locs(idelete) = [];         % peak locations

LMRpeakValue = pks;
LMRpeakLocations = locs;

NumPeaks = length(LMRpeakLocations);
sortedLocs = sort(LMRpeakLocations,'ascend');   

%% Plot
if 0
    figure;
    if ~isempty(LMRpeakLocations)
        plot( sigIdx, (inputSig), 'Marker','.'); hold on;
        plot(sigIdx(THRpeakLocation), (inputSig(THRpeakLocation)),'go');
        plot( sigIdx(LMRpeakLocations), (inputSig(LMRpeakLocations)),'rd');      
        plot( sigIdx, ones(1,nSamp)*threshold ,'--r');
        
    else        
        plot( sigIdx, (inputSig));
        title('No peaks','fontsize',10,'fontweight','bold');
    end 
...
function antiCycleAliasing(Param, n) % [1] 

% Two kinds of aliasing effects can appear due to sampling, i.e., aliasing in the spectral
% frequency domain, which is the overlapping of images of cyclic cumulant polyspectra (CCP) 
% with the same cycle frequency, and aliasing in the cycle frequency domain,
% which is the overlapping of images of the CCP with different cycle frequencies.
...
if rF ==0
    if rho < n-1
         error('ErrorTests:convertTest',...
            'The cycle aliasing occurs! \n ( Oversampling Factor >= p-1 )' )
    end
else
   [a, index]= find( 2*m/n < rF & rF <= (2*m+2)/n );
   % Example) n=8, rF= 0.3
   % >> 2*m/n < rF 
   % [ 1     1     0     0 ]
   % >> rF <= (2*m+2)/n 
...
%     if rho < n-1
%          error('ErrorTests:convertTest',...
%             'The cycle aliasing occurs! \n RollOffFactor should be ( rolloffFactor >= p-1 )' )
%     end
% elseif 0 < rF && rF <= 1/4
%     if rho < 9
%          error('ErrorTests:convertTest',...
%             'The cycle aliasing occurs! ' )
%     end
% elseif 1/4 < rF && rF <= 2/4
%     if rho < 11
%          error('ErrorTests:convertTest',...
%             'The cycle aliasing occurs! ' )
%     end
% elseif 2/4 < rF && rF <= 3/4
%     if rho < 13
%          error('ErrorTests:convertTest',...
%             'The cycle aliasing occurs! ' )
%     end
% elseif 3/4 < rF && rF <= 4/4
%     if rho < 15
%          error('ErrorTests:convertTest',...
%             'The cycle aliasing occurs! ' )
%     end 
