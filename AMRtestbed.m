...
%% Date          History
% 150112        C 구현을 위해 awgn 함수 주석처리
% 150114        입력데이터를 mat파일로 자동 저장하게함
%               입력데이터 길이제한
%               인덱스가 배열의 크기를 넘는지 항상 확인할 것
% 150115        FFT 길이 임의로 조절              
% 150121        line 2391 에 정규화된 참조데이터를 mat파일로 저장하는 코드 추가
%               line 2909 
%               line 641  입력 IQ 데이터 저장 코드
% 150330        주파수 톤간격 추정 재표본화 단계 이후로 변경
...
%         if isFSK>0 % 심볼율 선 스펙트럼의 크기에 따라 FSK 분류 됬지만
                 % 톤 스펙트럼 검출에 실패한 경우
